...
// Attribution + cache-source — drives the "Generated by … • model" line
// and the "Loaded from team cache" badge in the walkthrough header.
...
<div class="walkthrough">
	...
	{:else if summary}
		...
		>
			<!-- Attribution row — surfaces who originally generated this
			     walkthrough and the AI provider config that produced it.
			     Renders identically for local-generated and cache-hit
			     walkthroughs; an extra badge appears when the row was
			     hydrated from the team cache. -->
			{#if generatedBy || providerConfig}
				<div class="walkthrough-attribution">
					{#if generatedBy?.avatarUrl}
						<img
							class="walkthrough-attribution__avatar"
							src={generatedBy.avatarUrl}
							alt=""
							width={16}
							height={16}
						/>
					...
					{#if generatedBy?.githubLogin}
						<span>Generated by <strong>@{generatedBy.githubLogin}</strong></span>
					{/if}
					{#if providerConfig}
						<span class="walkthrough-attribution__dot">·</span>
						<span>{providerConfig.model}</span>
						{#if providerConfig.thinkingEffort}
							<span class="walkthrough-attribution__dot">·</span>
							<span>thinking: {providerConfig.thinkingEffort}</span>
						{/if}
					{/if}
					{#if walkthroughSource === 'remote'}
						<span class="walkthrough-attribution__badge">
							Loaded from team cache
						</span>
					{/if}
				</div>
			{/if}
...
<style>
	...
	.walkthrough-content > .walkthrough-attribution,
	.walkthrough-content > :global([data-slot="separator"]) {
		grid-column: 3;
	}

	/* Attribution row — quiet utility line above the issues block. */
	...
	.walkthrough-attribution__avatar {
		width: 16px;
		height: 16px;
		border-radius: 50%;
	}
	.walkthrough-attribution__dot {
		opacity: 0.5;
	}
	.walkthrough-attribution__badge {
		margin-left: auto;
		padding: 2px 8px;
		border-radius: 999px;
		background: var(--color-bg-elevated, rgba(120, 200, 255, 0.1));
