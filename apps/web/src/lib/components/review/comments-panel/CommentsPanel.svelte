...
const threadsWithReplies = $derived(
  threads.filter((t) => (getThreadMessages(t.id)?.length ?? 0) > 1),
);
...
function toggleExpandAll(): void {
  const anyOpen = expandAll === true || visibleGroups.some((g) => rowOpen[g.filePath] === true);
  expandAll = !anyOpen;
  rowOpen = {};
}
