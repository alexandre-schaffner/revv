...
// any `$derived` in a component that calls a getter (e.g.
// `$derived(getIsStreaming())`) inherits a dependency on `_active` via
// Svelte's reactive graph — no manual version counter needed.
//
// Previously `active()` was a plain function. Svelte 5's dependency
// tracking through plain-function → Map.get() across module boundaries
// could silently keep a stale cached result, manifesting as the floating
// action buttons stuck on "Stop" until a tab switch forced the $derived
// to re-evaluate. Making it a `$derived` turns it into a proper signal
// node that Svelte always invalidates when `entries` or `activePrId`
// change.
...
function _getOrCreateEntry(prId: string): WalkthroughEntry {
