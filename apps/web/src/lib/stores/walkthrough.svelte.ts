...
export interface WalkthroughEntry {
   ...
   *   • `'local'`  — generated by the agent on this machine.
   *   • `'remote'` — hydrated from the team cache.
   * Drives the "Loaded from team cache" badge in the walkthrough header.
   */
  source: "local" | "remote";
  /**
...
/** `'local'` (generated here) | `'remote'` (hydrated from team cache). */
export function getSource(): "local" | "remote" {
  return _active?.source ?? "local";
}
/** GitHub identity of whoever generated this walkthrough. Null when unknown. */
export function getGeneratedBy(): WalkthroughEntry["generatedBy"] {
  return _active?.generatedBy ?? null;
}
/** AI provider config snapshot from the original run. Null when unknown. */
...
/**
 * Mark an entry as having been hydrated from the remote team cache. Fired
 * by the server's `walkthrough:cache-hit` envelope just before the
 * subsequent `walkthrough:complete` triggers a fresh fetch. Purely
 * cosmetic — the "Loaded from team cache" badge keys off `source`.
 */
export function onWalkthroughCacheHit(prId: string, walkthroughId: string, source: "remote"): void {
  const entry = store.entries.get(prId);
  if (entry) {
    updateEntry(prId, (e) => {
      e.source = source;
  ...
  // No entry yet — drop a partial entry so the subsequent fetch can
  // merge into it. The fetch will populate everything else.
  const placeholder = freshEntry();
  placeholder.source = source;
  placeholder.walkthroughId = walkthroughId;
  placeholder.isStreaming = false;
  setEntry(prId, placeholder);
}
