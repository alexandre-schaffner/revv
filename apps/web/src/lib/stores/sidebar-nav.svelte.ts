...
export function setFocusedId(id: string | null): void {
  ...
  if (id) {
    requestAnimationFrame(() => {
      const el = document.querySelector<HTMLElement>(`[data-sidebar-nav="${CSS.escape(id)}"]`);
...
function getNavItems(): HTMLElement[] {
  return Array.from(document.querySelectorAll<HTMLElement>("[data-sidebar-nav]"));
}

function getFocusedEl(): HTMLElement | null {
  if (!focusedId) return null;
  return document.querySelector<HTMLElement>(`[data-sidebar-nav="${CSS.escape(focusedId)}"]`);
}
...
export function moveDown(): void {
  ...
  const idx = items.findIndex((el) => el.getAttribute("data-sidebar-nav") === focusedId);
