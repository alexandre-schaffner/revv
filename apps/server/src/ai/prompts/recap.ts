...
4. Write the complete recap as your visible assistant response — start with the first heading and keep going to the end of "Project state". No preamble, no "Here is the recap:" framing, no inter-tool commentary. ONE continuous response, no tool calls in the middle.
5. Call commit_recap_overview ONCE with metadata only: the PR ids you actually included, the walkthrough ids you incorporated, and the pre-aggregated stats. The markdown body comes from the streaming buffer — do not pass it here. Match the stats numbers to what get_recap_state showed — do not freelance counts.
...
- Add an "Active work" section listing currently open PRs by author, target branch, title, and any walkthrough summary/risk level available. Sort by relevance: PRs with walkthroughs first, then recently updated. Page through every open PR via list_open_prs BEFORE you start writing the markdown — once you start, you can't make read calls without resetting the buffer.
...
- Call tools in this exact order:
  ...
  3. get_repo_context
  4. Write the recap markdown as your visible assistant response (no tool call — just type).
  5. commit_recap_overview (metadata only — the server reads the markdown from the buffer)
  6. complete_recap
- The visible-text composition step (4) is REQUIRED. If you skip straight from reads to commit_recap_overview, the buffer will be empty and the commit will reject with an error pointing you back at composition.
...
export function buildRecapUserMessage(
  ...
  return [
    ...
    ...(rerunHint ? ["", rerunHint] : []),
    "",
    `Begin by calling get_recap_state. Then (when openPrsTotal > 0) walk every page of list_open_prs until nextOffset is null. Then get_repo_context. After all reads complete, write the COMPLETE recap markdown as your visible assistant response in one continuous block — no further tool calls until you finish, because a tool call resets the streaming buffer. Then commit_recap_overview with metadata only (no \`overview\` argument — the server reads what you typed). Finally call complete_recap.`,
  ].join("\n");
}
