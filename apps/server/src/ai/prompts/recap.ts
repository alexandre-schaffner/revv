...
3. As you compose each major section, call append_recap_chunk with the markdown text and a section hint (shipped, active_work, project_state, or other). Call it 2–4 times — once per section. This streams the recap live to the UI as you write it.
4. Call set_recap_overview ONCE with: the complete assembled markdown body, the PR ids you actually included, the walkthrough ids you incorporated, and the pre-aggregated stats. Match the stats numbers to what get_recap_state showed — do not freelance counts.
5. Call complete_recap to finalize. Then stop.
...
- Add an "Active work" section listing currently open PRs by author, target branch, title, and any walkthrough summary/risk level available. Sort by relevance: PRs with walkthroughs first, then recently updated. If the list was capped, mention "Showing N of M open PRs."
...
- Call tools in this exact order:
  1. get_recap_state
  2. get_repo_context
  3. append_recap_chunk (2–4 times, once per major section)
  4. set_recap_overview
  5. complete_recap
...
export function buildRecapUserMessage(
  ...
  const openPrHint =
    bundle.openPrs.length > 0
      ? `${bundle.openPrs.length} open PR(s) available for the "Active work" section.`
      : "No open PRs at the moment — skip the 'Active work' section.";
  ...
  return [
    ...
    ...(rerunHint ? ["", rerunHint] : []),
    "",
    `Begin by calling get_recap_state. Then get_repo_context. Stream each section as you write it via append_recap_chunk (2–4 calls). Then commit the final markdown with set_recap_overview. Finally call complete_recap.`,
  ].join("\n");
}
