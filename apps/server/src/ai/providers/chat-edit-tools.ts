...
function stampLastEdited(db: Db, walkthroughId: string, actor: string): void {
...
function decodeRating(row: typeof walkthroughRatings.$inferSelect): WalkthroughRating {
...
function decodeBlock(row: typeof walkthroughBlocks.$inferSelect): WalkthroughBlock | null {
  try {
...
function decodeIssue(row: typeof walkthroughIssues.$inferSelect): WalkthroughIssue {
  const blockIds = parseBlockIds(row.blockIds);
  return {
    id: row.id,
...
const getWalkthroughForEditHandler: ChatEditToolHandler<GetWalkthroughForEditInput> = async (
  ctx,
) => {
...
const updateOverviewHandler: ChatEditToolHandler<UpdateOverviewInput> = async (ctx, input) => {
...
const addSemanticStepEditHandler: ChatEditToolHandler<AddSemanticStepEditInput> = async (
  ctx,
  input,
) => {
...
const updateSemanticStepHandler: ChatEditToolHandler<UpdateSemanticStepInput> = async (
  ctx,
  input,
) => {
...
const deleteSemanticStepHandler: ChatEditToolHandler<DeleteSemanticStepInput> = async (
  ctx,
  input,
) => {
  const active = resolveActiveWalkthroughId(ctx.db, ctx.prId);
...
const addBlockHandler: ChatEditToolHandler<AddBlockInput> = async (ctx, input) => {
...
const updateBlockHandler: ChatEditToolHandler<UpdateBlockInput> = async (ctx, input) => {
  if (blockContentVariantCount(input.content) !== 1) {
...
const deleteBlockHandler: ChatEditToolHandler<DeleteBlockInput> = async (ctx, input) => {
  const active = resolveActiveWalkthroughId(ctx.db, ctx.prId);
  if (!active) return fail("No complete walkthrough exists for this PR yet.");
  const walkthroughId = active.id;
...
const updateSentimentHandler: ChatEditToolHandler<UpdateSentimentInput> = async (ctx, input) => {
...
const updateRatingHandler: ChatEditToolHandler<UpdateRatingInput> = async (ctx, input) => {
  const active = resolveActiveWalkthroughId(ctx.db, ctx.prId);
  if (!active) return fail("No complete walkthrough exists for this PR yet.");
  const walkthroughId = active.id;
...
const deleteRatingHandler: ChatEditToolHandler<DeleteRatingInput> = async (ctx, input) => {
  const active = resolveActiveWalkthroughId(ctx.db, ctx.prId);
  if (!active) return fail("No complete walkthrough exists for this PR yet.");
  const walkthroughId = active.id;
...
const addIssueEditHandler: ChatEditToolHandler<AddIssueEditInput> = async (ctx, input) => {
  const active = resolveActiveWalkthroughId(ctx.db, ctx.prId);
  if (!active) return fail("No complete walkthrough exists for this PR yet.");
  const walkthroughId = active.id;
...
const updateIssueHandler: ChatEditToolHandler<UpdateIssueInput> = async (ctx, input) => {
  const active = resolveActiveWalkthroughId(ctx.db, ctx.prId);
  if (!active) return fail("No complete walkthrough exists for this PR yet.");
  const walkthroughId = active.id;
...
const deleteIssueHandler: ChatEditToolHandler<DeleteIssueInput> = async (ctx, input) => {
  const active = resolveActiveWalkthroughId(ctx.db, ctx.prId);
  if (!active) return fail("No complete walkthrough exists for this PR yet.");
  const walkthroughId = active.id;
...
const addIssueCommentEditHandler: ChatEditToolHandler<AddIssueCommentEditInput> = async (
  ctx,
  input,
) => {
...
const updateIssueCommentHandler: ChatEditToolHandler<UpdateIssueCommentInput> = async (
  ctx,
  input,
) => {
  const active = resolveActiveWalkthroughId(ctx.db, ctx.prId);
...
const deleteIssueCommentHandler: ChatEditToolHandler<DeleteIssueCommentInput> = async (
  ctx,
  input,
) => {
  const active = resolveActiveWalkthroughId(ctx.db, ctx.prId);
