		...
		tools: [
					...
					if (!emitter.state.summarySet) {
						return {
							content: [
								{
									type: "text" as const,
									text: "Error: call set_walkthrough_summary first.",
								},
							],
							isError: true,
						};
					...
					if (emitter.state.completed) {
						return {
							content: [
								{
									type: "text" as const,
									text: "Error: walkthrough already completed.",
								},
							],
							isError: true,
						};
			...
			tool(
				...
				"Add an annotated code block showing source code from a specific file. Use to highlight important code the reviewer should see.",
				{
					...
					annotation_position: z
						.enum(["left", "right"])
						.describe(
							"Which side to display the annotation relative to the code",
						),
				},
				async (args) => {
					if (!emitter.state.summarySet) {
			...
			tool(
				...
				"Add an annotated diff block showing changes in unified diff format. Use to highlight specific changes the reviewer should focus on.",
				{
					...
					patch: z
						.string()
						.describe("Unified diff patch text (with @@ hunk headers)"),
					annotation: z
						.string()
						.nullable()
						.describe(
					...
					annotation_position: z
						.enum(["left", "right"])
						.describe(
							"Which side to display the annotation relative to the diff",
						),
				},
				async (args) => {
					if (!emitter.state.summarySet) {
			...
			tool(
				...
				"Flag a structured concern or issue found in the PR. Call this for every concern you identify — security vulnerabilities, race conditions, missing tests, edge cases, breaking changes, etc. You can call this alongside your narrative markdown sections.",
				{
					...
					title: z
						.string()
						.describe("Short title of the concern (10 words max)"),
					description: z
						.string()
						.describe(
