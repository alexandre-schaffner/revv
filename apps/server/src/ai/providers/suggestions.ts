...
function buildUserMessage(input: GenerateSuggestionsInput): string {
	...
	if (input.walkthrough) {
		...
		const truncatedSentiment =
			sentiment.length > 300 ? `${sentiment.slice(0, 300)}…` : sentiment;
...
function sanitizePrompts(values: unknown[]): string[] {
	...
	for (const v of values) {
		...
		const capped = trimmed.length > MAX_PROMPT_LENGTH
			? `${trimmed.slice(0, MAX_PROMPT_LENGTH - 1)}…`
			: trimmed;
		out.push(capped);
...
async function generateViaClaude(
	...
	if (!parsed) {
		throw new Error(
			`claude returned unparseable output (len=${collected.length})`,
		);
	}
	return parsed;
}
...
async function generateViaOpencode(
	...
	if (!client) {
		throw new Error(
			"OpencodeSupervisor reports daemon-running but no client available",
		);
	}
	...
	try {
		...
		if (errObj) {
			throw new Error(
				`opencode agent error: ${JSON.stringify(errObj).slice(0, 200)}`,
			);
		}
		...
		if (!parsed) {
			throw new Error(
				`opencode returned unparseable output (len=${collected.length})`,
			);
		}
		return parsed;
	} finally {
...
export async function generateSuggestions(
	...
	try {
			...
			input.agent === "claude"
				...
				: (() => {
						if (!input.opencodeDeps) {
							throw new Error(
								"generateSuggestions: opencodeDeps required when agent='opencode'",
							);
						}
		...
		const result = await withTimeout(
			...
			`suggestions:${input.agent}`,
		);
		debug(
			"suggestions",
			`generated ${result.length} prompts via ${input.agent}/${input.model}`,
		);
		return result;
	} catch (err) {
