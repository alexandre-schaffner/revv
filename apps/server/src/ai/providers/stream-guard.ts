...
import {
  WALKTHROUGH_EXPLORATION_STALL_MS,
...
const PHASE_MESSAGES: Record<
  string,
  { phase: WalkthroughLifecyclePhase; message: string }
> = {
  exploration: {
    phase: "exploring",
...
export function guardWalkthroughStream(
  ...
  const inactivityMs =
    options?.inactivityTimeoutMs ?? WALKTHROUGH_INACTIVITY_TIMEOUT_MS;
  const explorationStallMs =
    options?.explorationStallMs ?? WALKTHROUGH_EXPLORATION_STALL_MS;
  const synthesize = options?.synthesizePhases ?? true;
  const label = options?.label ?? "guard";
  const firstEventMs =
    options?.firstEventTimeoutMs ?? WALKTHROUGH_FIRST_EVENT_TIMEOUT_MS;
      ...
      while (true) {
        ...
        if (synthesize) {
          if (event.type === "exploration" && !emittedExploringPhase) {
            ...
            yield {
              type: "phase" as const,
              data: PHASE_MESSAGES.exploration!,
            };
          }
        ...
        if (event.type === "exploration") {
          if (Date.now() - lastProgressTime > explorationStallMs) {
            debug(
              label,
              "Exploration stall — no progress for",
              explorationStallMs,
              "ms",
            );
            yield {
