...
export const blockContentSchema = z
	.object({
		...
		code: z
			...
			.optional()
			.describe(
				"Use for source-code excerpts. Mutually exclusive with `markdown` and `diff`.",
			),
		diff: z
			.object({
				file_path: z.string(),
			...
			.optional()
			.describe(
				"Use for unified-diff hunks. Mutually exclusive with `markdown` and `code`.",
			),
	})
	.describe(
...
export const updateBlockSchema = z.object({
	...
	step_index: z
		...
		.nonnegative()
		.describe(
			"Position of the existing block within its chapter. Must already exist.",
		),
	content: blockContentSchema,
});
...
export const updateIssueCommentSchema = z.object({
	thread_message_id: z
		.string()
		.describe("thread_messages.id of the comment to update."),
...
export type GetWalkthroughForEditInput = z.infer<
	typeof getWalkthroughForEditSchema
>;
