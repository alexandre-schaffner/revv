...
async function fetchGitHubUser(
	...
	const res = await fetch(urls.userUrl, {
		headers: { Authorization: `Bearer ${accessToken}`, Accept: 'application/json', 'User-Agent': 'Revv/1.0' },
	});
	if (!res.ok) {
...
async function retryFetch<T>(fn: () => Promise<T>, maxRetries: number, delayMs: number): Promise<T> {
...
async function upsertUserAndSession(
	...
	if (existingAccount) {
		await db
			.update(account)
			.set({ accessToken, updatedAt: now, userId, githubLogin: githubUser.login, avatarUrl: githubUser.avatar_url })
...
const publicAuthRoutes = new Elysia()
	.get('/api/auth/local-accounts', async () => {
		...
		const accounts = await db
			.select({ userId: account.userId, providerId: account.providerId, githubLogin: account.githubLogin, avatarUrl: account.avatarUrl })
			.from(account)
			.where(inArray(account.userId, userIds));

		const accountsByUserId = new Map<string, Array<{ host: string; githubLogin: string | null; avatarUrl: string | null }>>();
		for (const acc of accounts) {
			const host = acc.providerId.split(':')[1];
			if (!host) continue;
			const existing = accountsByUserId.get(acc.userId) ?? [];
			existing.push({ host, githubLogin: acc.githubLogin ?? null, avatarUrl: acc.avatarUrl ?? null });
	...
	.post(
		...
		async (ctx) => {
			...
			if (!targetUser) return ctx.status(404, { error: 'User not found or not set up on this machine' });
			...
			if (!activeSession) return ctx.status(404, { error: 'User not found or not set up on this machine' });
	...
	.post(
		...
		async ({ body, status }) => {
			...
			if (!res.ok) {
				...
				logError('device-auth', `GitHub device code request failed: ${res.status} ${res.statusText}`, text);
