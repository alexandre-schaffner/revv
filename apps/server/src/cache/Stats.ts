...
export class CacheStats extends Context.Tag("CacheStats")<
  CacheStats,
  {
    ...
    readonly snapshot: () => Effect.Effect<CacheNamespaceStats[], never, DbService>;
...
export const CacheStatsLive = Layer.effect(
  CacheStats,
  Effect.gen(function* () {
    const registry = yield* Ref.make<Map<string, NamespaceRegistration>>(new Map());
    ...
    const snapshot = () =>
      Effect.gen(function* () {
        ...
        for (const reg of m.values()) {
          const { entries, approxBytes } = yield* reg.backend.countEntries(reg.namespace);
          const { oldestAt, newestAt } = yield* reg.backend.bounds(reg.namespace);
          const { hits, misses, revalidatedUnchanged, inflightDedups, bytesSaved } = reg.counters;
      ...
      Effect.gen(function* () {
        ...
        return all.reduce(
          (acc, r) => ({
            ...
            revalidatedUnchanged: acc.revalidatedUnchanged + r.revalidatedUnchanged,
