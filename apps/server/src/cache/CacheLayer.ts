...
import type { CacheCounters, CacheRow, FetcherResult, RevalidationHint } from "./types";
...
export function makeCacheLayer<K, V>(
  ...
  return Effect.gen(function* () {
    ...
    const doParse: (json: string) => V = parse ?? ((json) => JSON.parse(json) as V);
    ...
    const set = (
      ...
      Effect.gen(function* () {
        ...
        const freshResult: FetcherResult<V> =
          meta === undefined ? { kind: "fresh", value: v } : { kind: "fresh", value: v, meta };
        const decision = policy.decideWrite<V>(freshResult, null, Date.now());
    ...
    const getOrFetch = <E, R>(
      ...
      Effect.gen(function* () {
        ...
        const hint: RevalidationHint<V> = {
          previousValue: previousRow ? tryParse(previousRow.valueJson) : null,
          previousEtag: previousRow?.etag ?? null,
          previousLastModified: previousRow?.lastModified ?? null,
          previousRow,
        };

        const writeResult = (result: FetcherResult<V>): Effect.Effect<V, CacheMiss, DbService> =>
          Effect.gen(function* () {
            const decision = policy.decideWrite<V>(result, previousRow, Date.now());
            if (decision.kind === "drop") {
              yield* backend.deleteOne(namespace, key);
        ...
        const materialized = fetcher(hint).pipe(Effect.flatMap(writeResult), Effect.either);
