 ...
 */
export type Keyer<K> = (key: K) => string;
...
export function makeTupleKeyer<K>(extract: (k: K) => readonly (string | number)[]): Keyer<K> {
  return (k) =>
    extract(k)
      .map((p) => String(p))
      .join("\0");
...
export function makeHashKeyer<K>(extract: (k: K) => readonly (string | number)[]): Keyer<K> {
