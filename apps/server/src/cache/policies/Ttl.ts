...
import { buildPersist, isoNow, type Policy, parseIso, type WriteDecision } from "./Policy";
...
export class TtlPolicy implements Policy {
  ...
  decideWrite<V>(result: FetcherResult<V>, _previous: CacheRow | null, now: number): WriteDecision {
    switch (result.kind) {
      case "invalid":
        return { kind: "drop" };
      case "unchanged":
