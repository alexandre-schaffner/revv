...
export interface Policy {
   ...
   * delete the row.
   */
  decideWrite<V>(result: FetcherResult<V>, previous: CacheRow | null, now: number): WriteDecision;
}
...
export function serializeValue<V>(value: V): { valueJson: string; approxBytes: number } {
  const valueJson = JSON.stringify(value);
  return { valueJson, approxBytes: approxBytes(valueJson) };
}

export function serializeTag(tag: Record<string, unknown> | undefined): string | null {
  if (!tag) return null;
  return JSON.stringify(tag);
}

export function readTag<T extends Record<string, unknown>>(tagJson: string | null): T | null {
