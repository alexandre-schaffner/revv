...
export const DiffCacheServiceLive = Layer.succeed(DiffCacheService, {
  getCachedFiles: (prId) =>
    Effect.gen(function* () {
      ...
      const rows = db
        .select()
        .from(prDiffFiles)
        .where(eq(prDiffFiles.prId, prId))
        .all();
  ...
  getPrIdsWithCachedDiffs: () =>
    Effect.gen(function* () {
      const { db } = yield* DbService;
      const rows = db
        .selectDistinct({ prId: prDiffFiles.prId })
        .from(prDiffFiles)
        .all();
