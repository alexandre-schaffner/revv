...
export const PollSchedulerLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fiberRef = yield* Ref.make<Fiber.RuntimeFiber<number, never> | null>(
      null,
    );
    ...
    const syncAllRepos: Effect.Effect<
      void,
      never,
    ...
    > = Effect.gen(function* () {
        ...
        (repo) =>
          Effect.gen(function* () {
            ...
            const fresh = yield* github
              .getRepoFresh(repo.fullName, token)
      ...
      if (existingPrs.length > 0) {
        ...
        for (const pr of allPrs) {
          const repoFullName =
            allRepos.find((r) => r.id === pr.repositoryId)?.fullName ??
            pr.repositoryId;
    ...
    const threadFiberRef = yield* Ref.make<Fiber.RuntimeFiber<
      number,
      never
    > | null>(null);
        ...
        (pr) =>
          withDb(syncService.syncThreads(pr.id)).pipe(
            ...
            Effect.catchAllCause((cause) =>
              Effect.sync(() => {
                console.error(
                  `[PollScheduler] Thread sync failed for PR ${pr.id}:`,
                  Cause.pretty(cause),
                );
              }),
            ),
    ...
    const startThreadFiber: Effect.Effect<void> = Effect.gen(function* () {
      const schedule = Schedule.spaced(
        Duration.seconds(THREAD_SYNC_INTERVAL_SECONDS),
      );
