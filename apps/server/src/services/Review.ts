...
export class ReviewService extends Context.Tag("ReviewService")<
  ReviewService,
  {
    ...
    readonly getThread: (threadId: string) => Effect.Effect<CommentThread, ReviewError, DbService>;
...
export const ReviewServiceLive = Layer.succeed(ReviewService, {
  ...
  createThread: (sessionId, params) =>
    Effect.gen(function* () {
      ...
      yield* tryDb("create thread", (db) => db.insert(commentThreads).values(row).run());
  ...
  updateThreadStatus: (threadId, status) =>
    Effect.gen(function* () {
      ...
      const isReopening =
        status === "open" || status === "pending_coder" || status === "pending_reviewer";
    ...
    Effect.gen(function* () {
      const rows = yield* tryDb("list messages", (db) =>
        db
          ...
          .from(threadMessages)
          .leftJoin(remoteUsers, eq(remoteUsers.login, threadMessages.authorLogin))
  ...
  setHunkDecision: (sessionId, filePath, hunkIndex, decision) => {
    ...
    return tryDb("set hunk decision", (db) =>
      db
        ...
        .onConflictDoUpdate({
          target: [hunkDecisions.reviewSessionId, hunkDecisions.filePath, hunkDecisions.hunkIndex],
