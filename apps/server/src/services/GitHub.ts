    ...
    try: async () => {
      ...
      if (res.status === 422) {
        ...
        throw new GitHubNetworkError({
          cause: `422 Unprocessable Entity: ${text}`,
        });
      }
      assertGitHubOk(res, path);
      return res.json();
...
function mapPr(
  ...
  const rawReviewers = raw.requested_reviewers as
    ...
    Effect.gen(function* () {
      ...
      return (data as Record<string, unknown>[]).map((raw): GhReviewComment => {
        ...
        return {
          ...
          line:
            (raw.line as number | null) ??
            (raw.original_line as number | null) ??
            null,
