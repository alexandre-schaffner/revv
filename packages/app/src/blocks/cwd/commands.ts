...
export const getCwd = createServerFn({ method: "GET" }).handler(async () => {
  ...
  // biome-ignore lint/style/noNonNullAssertion: run() always populates result
...
export const getGitRepos = createServerFn({ method: "GET" }).handler(async () => {
  // biome-ignore lint/style/noNonNullAssertion: run() always populates result
  const home = (await run(BLOCK, "sh", ["-c", "echo $HOME"])).result!.stdout.trim();

  const searchDirs = ["dev", "Developer", "projects", "src", "code", "repos", "workspace"].map(
    (d) => `${home}/${d}`,
  );
