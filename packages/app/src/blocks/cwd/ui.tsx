import { Popover, PopoverContent, PopoverTrigger } from "@revv/ui/components/ui/popover";
import { useEffect, useState } from "react";
import { getCwd, getGitRepos, setCwd } from "./commands";
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" side="top" className="w-auto max-w-md p-1">
        <div className="flex flex-col">
          {repos.length === 0 && (
            <span className="px-3 py-2 text-xs text-muted-foreground">Scanning…</span>
          )}
          {repos.map((repo) => (
            <button
              key={repo}
              ...
              className={`text-left px-3 py-1.5 text-xs font-mono rounded-sm hover:bg-accent hover:text-accent-foreground transition-colors ${
                repo === cwd ? "text-foreground font-medium" : "text-muted-foreground"
              }`}
            >
              {repo}
            </button>
