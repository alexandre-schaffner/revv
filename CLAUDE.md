...
**Motion: GSAP via `$lib/motion`.** All animations are driven by GSAP and go through the `$lib/motion` module (`apps/web/src/lib/motion/`). Use the presets and `use:` actions from there (`gsapIn`, `gsapPress`, `bitsAnim`, `panelSlideIn`, `dialogSpringIn`, …) instead of CSS `transition:` rules, Svelte built-in transitions (`transition:fade`, `transition:slide`, …), or hand-rolled `@keyframes`. The motion tokens (`--duration-*`, `--ease-*`) in `app.css`' `@theme` block are still the source of truth — but they're consumed by `$lib/motion/tokens.ts`, not by Tailwind motion utilities. Never import `gsap` from the package directly; only `$lib/motion/gsap.ts` may.

**Reduced-motion contract.** `gsap.matchMedia()` (via `withMotion` in `$lib/motion/match-media.ts`) is the single arbiter of `prefers-reduced-motion`. All actions and presets short-circuit to `gsap.set()` (end-state only) under reduced motion. A preset can opt back in by exporting `preset.essential = true` (loading spinner, stream cursor). The legacy global `@media (prefers-reduced-motion: reduce)` block in `app.css` and `.motion-essential-*` utility are scheduled for removal in Phase 7 of the migration; until then they provide defensive coverage for any not-yet-migrated CSS transition.
